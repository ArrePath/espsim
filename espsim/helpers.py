...
def check_hs(mol):
    ...
    """
    # More efficient: check if any atom has implicit hydrogens instead of
    # creating a full copy of the molecule with AddHs()
    for atom in mol.GetAtoms():
        if atom.GetNumImplicitHs() > 0:
            warnings.warn(
                "Input molecule missing hydrogens. Use Chem.AddHs() to make hydrogens explicit."
            )
            break
...
try:
    ...
    def psi4Charges(
        ...
        """
        raise ImportError(
            "Failed to import Psi4 and RESP. Please install via 'conda install -c psi4 psi4 resp'"
        )
finally:
...
def get_reorder_list(mol):
    ...
    if chemprop_mol.GetNumAtoms() != len(atom_order):
        raise ValueError(
            "Could not reorder partial charges for molecule of length",
            chemprop_mol.GetNumAtoms(),
            "with subgraph match of only length",
            len(atom_order),
        )
...
try:
    ...
    def mlCharges(mols):
        ...
        partial_charge = np.split(
            partial_charge.flatten(), np.cumsum(np.array(n_atoms))
        )[:-1]
    ...
    def mlCharges(mols):
        ...
        """
        raise ImportError(
            "Failed to import the atomic Chemprop model. Please install via 'pip install git+https://github.com/hesther/chemprop-atom-bond.git'"
        )
...
def readSdfFile(f, key="CHARGES"):
    ...
    if key not in mol.GetPropsAsDict().keys():
        raise ValueError(
            "Unknown property key supplied. Check choice of key and/or the supplied sdf file"
        )
    charge = list(mol.GetProp(key).split(","))
    if len(charge) != mol.GetNumAtoms():
        raise ValueError(
            "List of partial charges must contain exactly N_atoms entries."
        )
    return mol, charge
